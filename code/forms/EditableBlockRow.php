...
class EditableBlockRow extends RequestHandler implements GridField_HTMLProvider, GridField_SaveHandler, GridField_URLHandler, GridField_ColumnProvider
{
    ...
    public function getForm($grid, $record, $removeEditableColumnFields = true)
    {
        ...
        $form = \Form::create($this, 
                $grid->ID() . '-EditableRow-' . $record->ID,
                //\FieldList::create(),
                $this->getFieldList($record, $grid, $removeEditableColumnFields), 
                \FieldList::create(),
                $this->getValidatorForForm($record, $grid)
            )
            ->loadDataFrom($record)
            ->setFormAction($this->Link('form',$record->ID))
    ...
    protected function getFieldList($record, $grid = null, $removeEditableColumnFields = true)
    {
        ...
        if (!$fields && $record->hasMethod('getEditableRowFields')) {
            $fields = $record->getEditableRowFields($grid);
        }
